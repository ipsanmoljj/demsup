# =============================================================================
# intraday_signal_engine_v2.R  — Dual-layer intraday execution engine
#
...
#   Three triggers tested; IC computed on training data to rank them.
...
#
# Stop:   1.0x intraday ATR14 (tighter than daily 2.5x)
# Target: 2.0x stop distance
# Exit:   stop / target / session close
#
# Signal source: output/regime_labels_CL.csv  (covers 2021-2026)
# Data splits:   Train → Dec 2023 | Validation Jan-Jun 2024 | Test Jul 2024+
# =============================================================================
...
# Override hooks — set via run_backtest_v2(vwap_extend=, ema_tol=, gap_min=)
...
# Stop multiplier override — set externally via run_backtest_v2(atr_stop_mult = X)
...
.backtest_product <- function(prod_name, cfg, bars_all, signal_dt, split) {
  ...
  # IC scan on training data
  if (split == "train") {
    cat(sprintf("\n  [%s] IC scan on training signal days:\n", prod_name))
...
.print_summary <- function(all_trades) {
  ...
  test <- all_trades[split == "test"]
  if (nrow(test) > 0L) {
    cat("Test detail — by trigger:\n")
    print(test[, .(trades=.N,
                   hit=round(100*mean(pnl_pts>0),1),
                   pnl=round(sum(pnl_pts),3)),
               by=.(product,trigger_used)][order(product,-trades)])
    cat("\nTest detail — by exit reason:\n")
    print(test[, .(trades=.N,
                   hit=round(100*mean(pnl_pts>0),1),
                   pnl=round(sum(pnl_pts),3)),
...
# 9. Main entry point
# -----------------------------------------------------------------------------
run_backtest_v2 <- function(splits = c("train","validation","test"),
                            atr_stop_mult  = NULL,
...
#
# # Step 1 — baseline (defaults) on training data:
# r_base <- run_backtest_v2(splits = "train")
#
# # Step 2 — tighten triggers to raise hit rate (fewer, stronger setups):
# #   Higher vwap_extend = spread must be MORE stretched from VWAP before firing
# #   Lower  ema_tol     = price must be CLOSER to the EMA (tighter pullback)
# #   Higher gap_min     = gap must be BIGGER before counted as a setup
# r_tight <- run_backtest_v2(splits = "train",
#                            vwap_extend = 0.75,
#                            ema_tol     = 0.15,
#                            gap_min     = 0.6)
#
# # Step 3 — compare hit rate / trade count / P&L between base and tight:
# compare <- rbindlist(list(
#   r_base[,  .(set="base",  trades=.N, hit=round(100*mean(pnl_pts>0),1), pnl=round(sum(pnl_pts),3))],
#   r_tight[, .(set="tight", trades=.N, hit=round(100*mean(pnl_pts>0),1), pnl=round(sum(pnl_pts),3))]
# ))
# print(compare)
#
# # Step 4 — once satisfied on training, validate ONCE on test:
# results <- run_backtest_v2(splits = "test",
#                            vwap_extend = 0.75, ema_tol = 0.15, gap_min = 0.6)
# =============================================================================